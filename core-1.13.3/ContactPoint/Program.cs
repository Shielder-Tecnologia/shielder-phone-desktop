...
namespace ContactPoint
{
    static class Program
    {
        ...
        public static MainForm Armaenza { get; set; }
        ...
        static void Main(string[] args1)
        {
            ...
            if (args1.Any())
            {
                string[] numero = args1[0].Split('@', ':');

                string[] method = args1[0].Split('=');
            ...
            if (GetCommandLineSwitch("/?", armazena) || GetCommandLineSwitch("/h", armazena) || GetCommandLineSwitch("/help", armazena))
            {
            ...
            var logFile = GetCommandLineParameter("/log", armazena);
            if (logFile != null)
            {
                try
                {
            ...
            Logger.LogLevel = 5;
#endif
            if (int.TryParse(GetCommandLineParameter("/loglevel", armazena), NumberStyles.Any, null, out var logLevel))
            {
            ...
            DisableExceptionReporter = GetCommandLineSwitch("/DisableExceptionReporter", armazena);
            ...
            {
                Logger.LogNotice($"ContactPoint Shielder Phone version: {typeof(Program).Assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion}");
...
#if DEBUG
                if (armazena.Contains("/debugger", StringComparer.InvariantCultureIgnoreCase))
                {
                ...
                var makeCallMessage = StartPhoneCallCommand.CreateFromCommandLine(GetCommandLineParameter("/call", armazena));
                try
                {
                    try
                    {
                    ...
                    {
                        if (!GetCommandLineSwitch("/DisableSplashScreen", armazena))
                        {
                        ...
                        try
                        {
                            ...
                            using (var core = CoreLoader.CreateCore(AppContext.MainForm))
                            {
                                ...
                                using (new AudioDeviceService(core))
                                {
                                    ...
                                    AppContext.ContactPointForm.DisableSettingsFormAutoStartup = GetCommandLineSwitch("/DisableSettingsFormAutoStartup", armazena);
        ...
        {
            Console.WriteLine($@"ContactPoint Shielder Phone ({typeof(Program).Assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion})
Usage: ContactPoint.exe [options]

Start Shielder Phone application.
